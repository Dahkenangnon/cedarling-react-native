...
export default function App() {
  ...
  const initializeIfNeeded = useCallback(async () => {
    ...
    await trace('Asset.downloadAsync', { module: 'policy-store.cjar' }, () =>
    ...
    await trace(
      'initialize',
      { bootstrap, policyStore: { kind: 'archive', uri: archive.localUri } },
      () =>
        Cedarling.initialize({
          bootstrap,
          policyStore: { kind: 'archive', uri: archive.localUri as string },
        })
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView contentContainerStyle={styles.content}>
        <Text style={styles.eyebrow}>NATIVE AUTHORIZATION LAB</Text>
